...
private:
    ...
    int* S;
public:
    Stack(int size); // constructor to create a stack of given size
    ~Stack(); // destructor to delete the stack
    void push(int x); // push an element onto the stack
    int pop(); // pop an element from the top of the stack
    int peek(int index); // peek at an element at a given index from the top of the stack
    ...
    int stackTop(); // get the top element of the stack
};
...
int main() {
    ...
    Stack stk(sizeof(A)/sizeof(A[0])); // create a stack of size equal to the length of the array
